...
if (-not (Test-Path -LiteralPath $RuntimeEnv)) { throw "RUNTIME_ENV_NOT_FOUND: $RuntimeEnv" }
Get-Content -LiteralPath $RuntimeEnv | ForEach-Object {
    $line = $_.Trim(); if (-not $line -or $line.StartsWith("#")) { return }
    $parts = $line.Split('=', 2); if ($parts.Count -ne 2) { throw "INVALID_RUNTIME_ENV_LINE: $line" }
    [Environment]::SetEnvironmentVariable($parts[0], $parts[1], 'Process')
}
$required = @('RUNTIME_N_PLUS','FORCE_ACTIVE','RUNTIME_SCHEMA','RUNTIME_GENERATION','HOME_SOURCE_GATE_DRIVE_ID','R000_CURRENT_CARRIER_DRIVE_ID','FOUR_D_FOLDER_ID','FOUR_D_MATRIX_ID','FOUR_D_CONFIG_ID','FOUR_D_LIVING_LOG_ID','MCM_CURRENT','MCM_BASE','R000_HELD_ACROSS_TURN','R000_RECUR_ON_EVERY_DELTA','R000_PRESENT_AT_READBACK','MULTI_WORKSPACE_RUNTIME','MULTI_SLOT_IS_MULTI_TIMELINE','MULTI_SLOT_IS_MULTI_POINTER','ALL_OLD_RUNTIME','OLD_ROOT_RUNTIME')
foreach ($name in $required) { if ([string]::IsNullOrWhiteSpace([Environment]::GetEnvironmentVariable($name,'Process'))) { throw "RUNTIME_N_PLUS_MISSING: $name" } }
